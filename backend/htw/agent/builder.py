...
from htw.llm import LLMBuilderWithoutModel, sanitize_value
import random
import json
import os
...
def get_agents(llm_builder: LLMBuilderWithoutModel, num_agents: int) -> list[ArgumentaBot]:
    ...
    for character in selected_characters:
        system_message = sanitize_value(f"You are {character['Character']}, {character['Description']}. Your attributes are: Charisma: {character['Charisma']}, Intellect: {character['Intellect']}, Stubbornness: {character['Stubbornness']}, Empathy: {character['Empathy']}, Influence Range: {character['Influence Range']}, Loyalty: {character['Loyalty']}, Curiosity: {character['Curiosity']}, Consistency: {character['Consistency']}. After every message state as ""Status:"" if you want to stay on your current team, say neutral or join the character that you are talking toos team.\n\n{root_message}")
        agents.append(
            ArgumentaBot(
                name=sanitize_value(character['Character']),
                uuid=sanitize_value(character['UID']),
